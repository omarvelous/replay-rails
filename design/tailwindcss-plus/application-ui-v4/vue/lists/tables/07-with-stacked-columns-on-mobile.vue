          ...
          <tr v-for="person in people" :key="person.email">
            <td class="w-full max-w-0 py-4 pr-3 pl-4 text-sm font-medium text-gray-900 sm:w-auto sm:max-w-none sm:pl-0 dark:text-white">
              ...
              <dl class="font-normal lg:hidden">
                ...
                <dd class="mt-1 truncate text-gray-700 dark:text-gray-300">{{ person.title }}</dd>
                <dt class="sr-only sm:hidden">Email</dt>
                <dd class="mt-1 truncate text-gray-500 sm:hidden dark:text-gray-400">{{ person.email }}</dd>
              </dl>
            </td>
            <td class="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell dark:text-gray-400">{{ person.title }}</td>
            <td class="hidden px-3 py-4 text-sm text-gray-500 sm:table-cell dark:text-gray-400">{{ person.email }}</td>
            <td class="px-3 py-4 text-sm text-gray-500 dark:text-gray-400">{{ person.role }}</td>
