...
- **Provisioned vs browser** — presence of `app_version` distinguishes provisioned devices from browser players. Same registration endpoint (`POST /api/v1/players`).
- **API docs** — OpenAPI spec at `docs/api/openapi.yaml`, browsable at `play.replaytv.co/api/docs` via Swagger UI.
...
| Models | `.claude/standards/models/conventions.md` | Validations match schema, associations with dependent, scopes, callbacks, tenant scoping |
