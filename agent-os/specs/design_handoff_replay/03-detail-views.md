...
> Deep spec for the four most recently designed surfaces. Read `01-design-system.md` first — every token referenced here is defined there.
...
## Source files for these views

| File | Contains |
|---|---|
| `source/admin/js/screens-listing-detail.jsx` | `ListingDetail` page + `ListingEditor` modal |
| `source/admin/js/screens-detail-drawers.jsx` | Shared `Drawer` shell, `ScreenDetail`, `PlayerDetail` |
| `source/admin/admin.css` | `.ld-*` (listing detail), `.le-*` (listing editor), `.dw-*` (drawers) |
| `source/admin/js/screens-content.jsx` | Listings index — entry points into the detail page and editor |
| `source/admin/js/screens-screens.jsx` | Screens index + `ScreenContent`, the live-preview renderer reused in both drawers |
| `source/admin/js/screens-players.jsx` | Players index — drawer entry points |
| `source/assets/replay.css` | Design tokens |
