...
RSpec.describe "ScreenPairings", type: :request do
  ...
  describe "GET /screens/:screen_id/pairing/new" do
    it "returns a successful response" do
      get new_screen_pairing_path(screen)
      expect(response).to be_successful
    end
  end

  describe "POST /screens/:screen_id/pairing" do
    let(:player) { create(:player) }

    it "pairs the player to the screen" do
      post screen_pairing_path(screen), params: { code: player.pairing_code }
    ...
    it "redirects with alert for unknown code" do
      post screen_pairing_path(screen), params: { code: "XXXXXX" }
      expect(response).to redirect_to(new_screen_pairing_path(screen))
  ...
  describe "DELETE /screens/:screen_id/pairing" do
    it "unpairs the player" do
      player = create(:player)
      screen.pair_player!(player)

      delete screen_pairing_path(screen)
  ...
  describe "tenant isolation" do
    it "returns 404 for another account's screen" do
      ...
      get new_screen_pairing_path(other_screen)
