...
RSpec.describe "PlayerApi", type: :request do
  ...
  describe "GET /status" do
    ...
    it "returns paired: false when not paired" do
      get "/status", params: { code: player.pairing_code }
  ...
  describe "GET / (play)" do
    ...
    it "renders the playlist when one is assigned" do
      ...
      get "/", headers: { "Authorization" => "Bearer #{player.token}" }
      expect(response).to be_successful
    end
    ...
    it "renders unpaired when player has no screen" do
      ...
      get "/", headers: { "Authorization" => "Bearer #{unpaired_player.token}" }
      expect(response).to be_successful
  ...
  describe "POST /heartbeat" do
    ...
    it "updates last_heartbeat_at and device info" do
      post "/heartbeat", headers: { "Authorization" => "Bearer #{player.token}" }
      expect(response).to be_successful
