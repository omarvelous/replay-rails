...
### Experience (new — base model with delegated_type)

Mirrors the Ad pattern: Experience is the base model with
`delegated_type :experienceable`. `Experiences::ListingExperience`
is the first type. Future types (e.g., `Experiences::BrandExperience`)
...
class Experience < ApplicationRecord
  ...
  delegated_type :experienceable,
    types: %w[Experiences::ListingExperience],
    dependent: :destroy
...
**Columns (experiences table):**
- `account_id` (FK, not null)
- `experienceable_type` (string, not null)
- `experienceable_id` (bigint, not null)
- `name` (string, not null — "123 Main St Open House")
- `config` (jsonb, default: `{}`) — section toggles and settings

### Experiences::ListingExperience (new — type-specific model)
...
**Columns (listing_experiences table):**
...
**B2. Experience + ListingExperience models (TDD)**
- RED: Experience model spec — delegated_type :experienceable,
  has_many :screen_contents as contentable, validates name,
  listing/default_agent delegation
- RED: Experiences::ListingExperience model spec — belongs_to
  :listing, belongs_to :agent optional, default_agent method
- RED: Experience + ListingExperience factories
- GREEN: Experience model (delegated_type, account, name, config)
  + Experiences::ListingExperience model (listing, agent)
  + migrations for both tables
- Note: A1 already created the migrations and minimal models.
  This step fleshes out specs and the ListingExperience type.
- Commit: RED then GREEN
