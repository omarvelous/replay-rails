# Plan: Analytics with Ahoy + Governed Events (v3) — COMPLETE
...
1. **`redirect.followed` → `qr.scanned`** — replaced generic
   redirect event with QR-specific event carrying `qr_code_id`,
   `screen_content_id`, `ad_id`, `screen_id`.

2. **`current_user` / `current_account` pattern** — added public
   on-demand methods to Authentication concern that call
   `resume_session` lazily. Required because Ahoy's BaseController
   skips all before_action callbacks. Ahoy's default `current_user`
   now works without custom lambda.

3. **`account_id` on events from player** — passed as event
   property from templates, promoted to column by Store fallback.
   Player subdomain has no authenticated session.

4. **`ahoy.authenticate(user)` at sign-in** — sets user_id on
   visit. Account_id set via direct visit update since
   `authenticate` only handles user.

5. **`Ahoy.exclude_method`** — used for admin exclusion instead
   of `skip_before_action` or Store guard.

6. **`turbo:load`** — added listener for page view tracking on
   Turbo navigations (ahoy.trackView only fires on full loads).

7. **Impression model removed** — all queries migrated to
   `Ahoy::Event`. MetricSnapshot and MetricsRollupJob also removed.

8. **ahoy-email v4 API** — uses `has_history` + `track_clicks`,
   not `has_tracked_emails` (v3 API).

9. **Rollups gem** — `rollups` (plural), uses `group(:account_id)`
   for dimensions, `column: :time` for ahoy_events.

10. **`content.loaded`** — defined as PORO but not yet instrumented
    in JS. Deferred to when we need it.
